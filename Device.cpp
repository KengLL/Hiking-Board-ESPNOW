...
Device::Device() : userState(0), inbox(), carryMsg() {
    // Initialize peerList with two addresses
    // MAC: 30:ED:A0:A8:CE:D0, initials: "A1"
    // MAC: 48:27:E2:1E:55:B0, initials: "B2"
...
void Device::addOrUpdateCarryMsg(const MessageStruct& msg, int limit) {
...
void Device::addOrUpdateInboxIfPeer(const MessageStruct& msg) {
    ...
    if (msg.code == 99) return; // Don't add pairing messages to inbox
...
void Device::setUserState(int state) {
    userState = state;
}

int Device::getUserState() const {
    return userState;
}

// Device MAC address
void Device::setMACAddress(const uint8_t* mac) {
    macAddress.assign(mac, mac + 6);
}

const std::vector<uint8_t>& Device::getMACAddress() const {
    return macAddress;
}
...
void Device::addPeer(const uint8_t* macAddress, const std::string& initials) {
    ...
    PeerInfo info;
    info.mac = std::vector<uint8_t>(macAddress, macAddress + MAC_SIZE);
...
//     std::vector<uint8_t> macVec(macAddress, macAddress + MAC_SIZE);
...
bool Device::isPeer(const std::vector<uint8_t>& mac) const {
    for (const auto& peer : peerList) {
        if (peer.mac == mac) {
            return true;
        }
